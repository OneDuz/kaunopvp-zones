...
local redZone = { coords = vector3(162.1389, -1298.3722, 29.2186), radius = 100.0 }
...
AddEventHandler('esx:onPlayerDeath', function(data)
    if isInZone then
        Wait(1000)
        local safeCoords = GetSafeRespawnCoords(redZone, 10)
...
CreateThread(function()
    while true do
        ...
        if IsPedArmed(cache.ped, 4) then
            if not IsPedInCover(cache.ped, true) and not IsPedInHighCover(cache.ped) then
                DisableControlAction(0, 140, true)
                DisableControlAction(0, 141, true)
                DisableControlAction(0, 142, true)
                

                if IsPedShooting(cache.ped) and isInZone then
                    local playerWeapon = GetSelectedPedWeapon(cache.ped)
                    local weaponType = GetWeapontypeGroup(playerWeapon)

                    if weaponType == 416676503 then -- PISTOL
                        ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', 0.08)
                    elseif weaponType == 970310034 then -- RIFLE
                        ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', 0.06)
                    elseif weaponType == -957766203 then -- SMG
                        ShakeGameplayCam('SMALL_EXPLOSION_SHAKE', 0.08)
                    end
                end
            else
